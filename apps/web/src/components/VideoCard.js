...
export function VideoCard({ video }) {
    ...
    return (<Link href={`/watch/${video.id}`}>
      <div className="group cursor-pointer">
        <div className="relative overflow-hidden rounded-lg bg-gray-900 aspect-video">
          <Image src={video.thumbnail?.url || '/placeholder.jpg'} alt={video.title} fill className="object-cover group-hover:scale-105 transition-transform duration-300"/>
          {video.status !== 'READY' && (<div className="absolute inset-0 bg-black/60 flex items-center justify-center">
