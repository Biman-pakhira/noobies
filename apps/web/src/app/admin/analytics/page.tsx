    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-lg p-6">
            ...
            <div className="space-y-3">
              {platformAnalytics.data?.videoStatus?.map((item: any) => (
                <div key={item.status}>
                  ...
                  <div className="w-full bg-gray-800 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${(item._count / (platformAnalytics.data?.videoStatus?.reduce((sum: number, s: any) => sum + s._count, 0) || 1)) * 100}%`,
                      }}
                    ></div>
                  </div>
                </div>
      ...
      {!userAnalytics.error && (
        <div className="bg-gray-900 border border-gray-700 rounded-lg p-6">
          ...
          {!userAnalytics.data?.topUploaders || userAnalytics.data.topUploaders.length === 0 ? (
            <p className="text-gray-400">No uploader data available</p>
          ) : (
            <div className="space-y-2">
              {userAnalytics.data.topUploaders.slice(0, 10).map((uploader: any, index: number) => (
                <div key={uploader.uploaderId} className="flex items-center justify-between p-3 bg-gray-800/50 rounded">
