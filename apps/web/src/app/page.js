      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex gap-4 border-b border-gray-800 overflow-x-auto">
            <button onClick={() => setActiveTab('home')} className={`pb-2 px-4 font-semibold transition whitespace-nowrap ${activeTab === 'home'
            ? 'text-red-500 border-b-2 border-red-500'
            : 'text-gray-400 hover:text-white'}`}>
              Home
            </button>
            <button onClick={() => setActiveTab('trending')} className={`pb-2 px-4 font-semibold transition whitespace-nowrap ${activeTab === 'trending'
            ? 'text-red-500 border-b-2 border-red-500'
            : 'text-gray-400 hover:text-white'}`}>
              Trending
            </button>
            {user && (<button onClick={() => setActiveTab('recommendations')} className={`pb-2 px-4 font-semibold transition whitespace-nowrap flex items-center gap-2 ${activeTab === 'recommendations'
                ? 'text-red-500 border-b-2 border-red-500'
                : 'text-gray-400 hover:text-white'}`}>
                <span>🤖</span>
                For You
        ...
        {activeTab === 'recommendations' && !user && (<div className="bg-gray-900 border border-gray-800 rounded-lg p-6 mb-8">
            <p className="text-gray-400 mb-4">
              Sign in to get personalized video recommendations powered by AI.
            </p>
            <Link href="/login" className="inline-block px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition">
              Sign In
            </Link>
          </div>)}
        ...
        {isLoading ? (<div className="flex justify-center items-center h-64">
          ...
          </div>) : displayVideos && displayVideos.length > 0 ? (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
