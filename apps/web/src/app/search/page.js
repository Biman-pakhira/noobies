    ...
    return (<div className="min-h-screen bg-gray-950">
      ...
      <div className="sticky top-16 z-20 bg-gray-900 border-b border-gray-800 p-4">
        <div className="max-w-6xl mx-auto flex gap-4">
          <input type="text" value={filters.q} onChange={(e) => handleSearch(e.target.value)} placeholder="Search videos..." className="flex-1 bg-gray-800 text-white px-4 py-2 rounded-lg border border-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500"/>
          <button onClick={() => setShowFilters(!showFilters)} className="px-4 py-2 bg-gray-800 text-white rounded-lg hover:bg-gray-700 transition flex items-center gap-2">
            <span>⚙️</span>
            Filters
          </button>
        </div>
      ...
      {/* Filters */}
      {showFilters && (<div className="bg-gray-900 border-b border-gray-800 p-4">
          <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-4">
            {/* Sort */}
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Sort By</label>
              <select value={filters.sort || 'relevance'} onChange={(e) => handleFilterChange('sort', e.target.value)} className="w-full bg-gray-800 text-white px-3 py-2 rounded border border-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500">
            ...
            <div>
              ...
              <select value={filters.dateRange || ''} onChange={(e) => handleFilterChange('dateRange', e.target.value)} className="w-full bg-gray-800 text-white px-3 py-2 rounded border border-gray-700 focus:outline-none focus:ring-2 focus:ring-red-500">
            ...
            <div>
              ...
              <select value={filters.minDuration === 0 && filters.maxDuration === 240
                ? '0-4min'
      ...
      <div className="max-w-6xl mx-auto p-4">
        {!filters.q || filters.q.trim().length === 0 ? (<div className="text-center py-20">
            <p className="text-gray-400 text-lg">Enter a search query to get started</p>
          </div>) : isLoading ? (<div className="text-center py-20">
            <div className="inline-block animate-spin">
              <div className="w-8 h-8 border-4 border-gray-700 border-t-red-500 rounded-full"></div>
            </div>
          </div>) : results.length === 0 ? (<div className="text-center py-20">
            ...
            <div className="space-y-2 mb-8">
              {results.map((video) => (<Link key={video.id} href={`/watch/${video.id}`} className="group flex gap-4 p-3 bg-gray-900 rounded-lg hover:bg-gray-800 transition">
                  {/* Thumbnail */}
                  <div className="flex-shrink-0 relative w-40 h-24 bg-gray-800 rounded overflow-hidden">
                    {video.thumbnail ? (<img src={video.thumbnail} alt={video.title} className="w-full h-full object-cover group-hover:scale-105 transition"/>) : (<div className="w-full h-full bg-gray-700 flex items-center justify-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                      ...
                      {video.score && (<span className="text-red-500">
            ...
            {/* Pagination */}
            {total > 20 && (<div className="flex items-center justify-center gap-4 py-8">
                <button onClick={() => setPage((prev) => Math.max(prev - 1, 1))} disabled={page === 1} className={clsx('px-4 py-2 rounded font-medium transition', page === 1
                ...
                <button onClick={() => setPage((prev) => prev + 1)} disabled={page >= Math.ceil(total / 20)} className={clsx('px-4 py-2 rounded font-medium transition', page >= Math.ceil(total / 20)
                    ? 'bg-gray-800 text-gray-600 cursor-not-allowed'
                    : 'bg-red-600 text-white hover:bg-red-700')}>
                  Next
                </button>
