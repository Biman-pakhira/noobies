 ...
 */
async function removeInteraction(request, reply) {
    try {
        await request.jwtVerify();
        if (!request.user) {
            return reply.status(401).send({
 ...
 */
export async function registerInteractionRoutes(fastify) {
    fastify.post('/api/videos/:id/interact', createInteraction);
    fastify.delete('/api/videos/:id/interact', removeInteraction);
}
