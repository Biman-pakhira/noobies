 ...
 */
async function searchVideos(request, reply) {
    try {
        ...
        const uploaderId = request.query.uploaderId;
        const minDuration = request.query.minDuration ? parseInt(request.query.minDuration, 10) : null;
        const maxDuration = request.query.maxDuration ? parseInt(request.query.maxDuration, 10) : null;
        const dateRange = request.query.dateRange; // last_week, last_month, last_year
        const sortBy = request.query.sort || 'relevance'; // relevance, newest, mostViewed, trending
 ...
 */
async function getAutocompleteSuggestions(request, reply) {
    try {
        const esService = getElasticsearchService();
        if (!esService.isHealthy()) {
            return reply.status(503).send({
 ...
 */
async function getTrendingSearches(request, reply) {
    try {
        const esService = getElasticsearchService();
        if (!esService.isHealthy()) {
            return reply.status(503).send({
