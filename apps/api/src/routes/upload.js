 ...
 */
export async function registerMultipartPlugin(fastify) {
...
async function uploadVideo(request, reply) {
    try {
        ...
        for await (const part of parts) {
            if (part.type === 'file') {
                ...
                if (part.fieldname === 'title') {
                    ...
                    tagNames = part.value.split(',').map((t) => t.trim());
                }
            }
        }
        // Validate input
        ...
        if (tagNames.length > 0) {
            ...
            const connectedTags = await Promise.all(tagNames.map(async (tagName) => {
 ...
 * Get upload status
 */
async function getUploadStatus(request, reply) {
    try {
 ...
 */
export async function registerVideoUploadRoutes(fastify) {
    await registerMultipartPlugin(fastify);
    fastify.post('/api/videos/upload', { onRequest: [fastify.authenticate] }, uploadVideo);
    fastify.get('/api/videos/upload/:jobId', getUploadStatus);
    fastify.delete('/api/videos/:id', deleteVideo);
}
